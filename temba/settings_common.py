...
LLM_TYPES = {
    ...
    "temba.ai.types.custom_ai.type.CustomAIType": {
        # any OpenAI-compatible endpoint, e.g. a local AI gateway; the endpoint lives in the model's config
...
# the bridge's API, the admin token that lets the claim page create a bridge user, and the courier the bridge
# posts inbound webhooks to (courier is on the same host as the bridge, so this is a local URL, not the brand domain)
WUZAPI_URL = "http://localhost:8095"
WUZAPI_ADMIN_TOKEN = None
WUZAPI_COURIER_URL = "http://localhost:8080"
