...
class FlowRevisionTest(TembaTest):
    def test_validate_legacy_definition(self):
        ...
        validate(self.load_json("flows/legacy/invalid/not_fully_localized.json"), "non-localized flow definition")

        # base_language of null, but spec version 8
        validate(self.load_json("flows/legacy/invalid/no_base_language_v8.json"), "non-localized flow definition")

        # base_language of 'eng' but non localized actions
        validate(
            self.load_json("flows/legacy/invalid/non_localized_with_language.json"),
            "non-localized flow definition",
        )

        validate(self.load_json("flows/legacy/invalid/non_localized_ruleset.json"), "non-localized flow definition")
