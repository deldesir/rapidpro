...
class Command(BaseCommand):
    ...
    def generate_and_dump(self, specs_file, locs_file, mr_port: int, db_name, db_user, dump_file):
        ...
        mr_cmd = f'go run ./cmd/mailroom --internal-port={mr_port} -db="postgres://{db_user}:temba@localhost/{db_name}?sslmode=disable" --valkey="valkey://localhost:6379/15" --workers-realtime=0 --workers-batch=0 --workers-throttled=0 -uuid-seed=123'
